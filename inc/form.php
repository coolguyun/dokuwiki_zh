...
function form_wikitext($attrs) {
    // mandatory attributes weily
    unset($attrs['name']);
    unset($attrs['id']);
 //   return '<textarea name="wikitext" id="wiki__text" dir="auto" '.buildAttributes($attrs,true).'>'.DOKU_LF.formText($attrs['_text']).'</textarea>';
    $ueditor =  '<script type="text/javascript" charset="utf-8" src="'.DOKU_BASE.'ueditor/ueditor.config.js"></script>';
    $ueditor = $ueditor . '<script type="text/javascript" charset="utf-8" src="'.DOKU_BASE.'ueditor/ueditor.all.js"></script>';
    $ueditor = $ueditor . '<textarea name="wikitext" id="wiki__text" dir="auto" '.buildAttributes($attrs,true).'>'.DOKU_LF.formText($attrs['_text']).'</textarea>';
    if(!strstr($attrs['_text'], 'usemarkdown')){
    	 $ueditor = $ueditor . '<script type="text/javascript"> var ue = UE.getEditor(\'wiki__text\'); </script>';
    }
    return $ueditor;
}
